// SitePulse V3 — DeepSeek Provider Adapter
// Implements AIProvider for DeepSeek API (OpenAI-compatible).
...
export class DeepSeekAdapter extends AIProvider {
  ...
  constructor(config: { apiKey: string; baseUrl?: string; defaultModel?: string }) {
    ...
    this._baseUrl = config.baseUrl ?? 'https://api.deepseek.com/v1';
  ...
  async analyzeCode(code: string, context: CodeContext): Promise<CodeAnalysis> {
    const messages: AIMessage[] = [
      {
        role: 'system',
        content: `You are a code analysis engine for ${context.language} files. Return JSON: {issues: [{id, code, title, severity, route, evidence, timestamp}], suggestions: string[], confidence: number}. No markdown.`,
      },
      {
        role: 'user',
        content: `Analyze ${context.filePath}:\n\n${code}\n\nContext:\n${context.surrounding}`,
      },
    ];
  ...
  async generatePatch(finding: Finding, projectContext: string): Promise<Patch> {
    const messages: AIMessage[] = [
      {
        role: 'system',
        content: 'Generate a unified diff patch to fix the described issue. Return JSON: {diff: {hunks, fromFile, toFile, summary}, description}. No markdown.',
      },
      {
        role: 'user',
  ...
  async explainFinding(finding: Finding, language: string): Promise<string> {
    ...
    const messages: AIMessage[] = [
      {
        role: 'system',
        content: `Explain this issue concisely in ${langMap[language] ?? language}. Include business impact.`,
      },
      {
        role: 'user',
